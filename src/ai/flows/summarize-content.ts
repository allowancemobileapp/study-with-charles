...
// import type { PromptData } from 'genkit'; // Removed problematic import
...
const SummarizeContentOutputSchema = z.object({
  ...
  imageUrl: z.string().url().optional().describe("An optional URL to an image generated by the AI, if relevant to the result.")
});
...
const summarizeContentFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const promptMessages: ({text: string} | {media: {url: string}})[] = []; // Adjusted type
    let mainInstruction = "";
...
Ensure the JSON syntax is perfect, including correct use of quotes for all keys and string values, commas between objects (but not after the last object in the array), and proper brackets and braces.`;
    ...
    try {
      ...
      const response = await ai.generate({
        prompt: promptMessages, // Use the adjusted type for promptMessages
